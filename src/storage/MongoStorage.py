...
class MongoStorage:
	...
	def get_all_docs(self):
		"""
		Retrieves all documents from the 'documents' collection,
		converting them to a JSON-serializable list.
		"""
		try:
			# .find() returns a cursor, so we wrap it in list() to get all results at once.
			documents_cursor = self.db["documents"].find({})
			
			results = []
			for doc in documents_cursor:
				# Convert the MongoDB ObjectId to a string, which is required for JSON serialization.
				doc['_id'] = str(doc['_id'])
				results.append(doc)
	...
	def delete_all_docs(self):
		"""
		Deletes all documents from the 'documents' collection.
		...
		try:
			...
			logger.info(f"Deleted {deleted_count} documents from MongoDB collection 'documents'.")
